...
# beta.list = c(-20,-5,0,10, 30,40,50)

# beta.list = c(-15:15)
# beta.list = .1*(1:20)

# relationship between K and x1
# c.list = .25*(0:40)
...
for(i in 1:length(beta.list)){
  for(j in 1:length(beta.list)){
    beta.list.2[[length(beta.list)*(i-1) + j]] = c(beta.list[i],beta.list[j])
  }
}
...
sim = function(beta){
  ...
  for(i in 1:n.sim){
    ...
    # # fit outcome model doubly weighted with K in prop scope
    # outcome = svyglm(y.b ~ T.b + propscore.K, 
    #                  design = survey::svydesign(ids = ~0, weights = data.use$htweight, data = data.use),
    #                  family = stats::gaussian)
    # coef.wt2.Kprop[i,] = outcome$coefficients
    # 
    # # fit outcome model doubly weighted with K in prop scope
    # outcome = svyglm(y.b ~ T.b + propscore.nowt.K, 
    #                  design = survey::svydesign(ids = ~0, weights = data.use$htweight, data = data.use),
    #                  family = stats::gaussian)
    # coef.wt.outcome.Kprop[i,] = outcome$coefficients
    # 
    # # fit outcome model (weights only in outcome model) K in wt scope
    # outcome = svyglm(y.b ~ T.b + propscore.nowt, 
    #                  design = survey::svydesign(ids = ~0, weights = data.use$htweightK, data = data.use),
    #                  family = stats::gaussian)
    # coef.wt.outcome.Kwt[i,] = outcome$coefficients
    # 
    # # fit outcome model double weighted with K in sampling wt scope
    # outcome = svyglm(y.b ~ T.b + propscore.Kwt, 
    #                  design = survey::svydesign(ids = ~0, weights = data.use$htweightK, data = data.use),
    #                  family = stats::gaussian)
    # coef.wt2.Kwt[i,] = outcome$coefficients
...
simresults = sfLapply(beta.list.2,sim)
